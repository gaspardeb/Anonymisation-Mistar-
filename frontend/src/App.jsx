...
export default function App() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-cream-100 text-ink-400 text-sm">
        Chargement…
      </div>
    );
  }
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/" element={user ? <Layout /> : <Navigate to="/login" replace />}>
            ...
            <Route path="admin"      element={user?.role === 'admin' ? <Admin />  : <Navigate to="/anonymizer" replace />} />
          </Route>
        </Routes>
      </BrowserRouter>
